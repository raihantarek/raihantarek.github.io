...
document.getElementById("startDateLabel").textContent = startValid 
  ...
  function tick() {
    ...
    updateFunFact(totalHours);
  }

  function updateFunFact(totalHours) {
    var fact = cfg.funFact;
    var section = document.getElementById("factSection");
    if (!fact || !fact.template) { section.hidden = true; return; }
    var perUnit = fact.hoursPerUnit > 0 ? fact.hoursPerUnit : 1;
    var value = Math.max(0, Math.floor(totalHours / perUnit)).toLocaleString();
    document.getElementById("factLead").textContent = fact.lead || "";
    document.getElementById("factValue").textContent = fact.template.replace("{value}", value);
    if (fact.icon) document.querySelector(".fact__icon").textContent = fact.icon;
  }
  ...
  */
  ...
  (function initPlayer() {
    ...
    // Try to autoplay as soon as the page loads. Browsers block unmuted
    // autoplay before the visitor has interacted with the page at all,
    // so if that happens we just start it on the very first tap/click/
    // key press anywhere on the page instead — no button needed.
